...
from src.utils.utils import psd_solve, symmetrize
from src.parameters import ParameterProperties
...
def preprocess_params(params, num_timesteps):
...
def preprocess_args(f):
    ...
    def wrapper(*args, **kwargs):
        ...
        full_params = preprocess_params(params, num_timesteps)

        return f(full_params, emissions, condition=condition, trial_id=trial_id)
    return wrapper
...
def lgssm_joint_sample(
    ...
    """
    params = preprocess_params(params, num_timesteps)

    def _sample_transition(key, F, b, Q, x_tm1):
        mean = F @ x_tm1 + b
        return MVN(mean, Q).sample(seed=key)

    def _sample_emission(key, H, R, x):
        mean = H @ x
        R = jnp.diag(R) if R.ndim==1 else R
        return MVN(mean, R).sample(seed=key)

    def _sample_initial(key, params):
        key1, key2 = jr.split(key)

        initial_state = MVN(params.initial.mean[condition], params.initial.cov[condition]).sample(seed=key1)

        H0, R0 = _get_params(params, num_timesteps, 0)[2:]

        initial_emission = _sample_emission(key2, H0, R0, initial_state)
    ...
    def _step(prev_state, args):
        ...
        F, b, Q, H, R = _get_params(params, num_timesteps, t)

        # Sample from transition and emission distributions
        state = _sample_transition(key1, F, b, Q, prev_state)
        emission = _sample_emission(key2, H, R, state)
    ...
    initial_state, initial_emission = _sample_initial(key1, params)

    # Sample the remaining emissions and states
    next_keys = jr.split(key2, num_timesteps - 1)
    next_times = jnp.arange(1, num_timesteps)
    _, (next_states, next_emissions) = lax.scan(_step, initial_state, (next_keys, next_times))
...
def lgssm_filter(
    ...
    def _step(carry, t):
        ...
        F, b, Q, _, R = _get_params(params, num_timesteps, t)
        y = emissions[t]

        # Update the log likelihood
        ll += _log_likelihood(_pred_mean, _pred_cov, H, R, y)

        # Condition on this emission
        filtered_mean, filtered_cov = _condition_on(_pred_mean, _pred_cov, H, R, y)

        # Predict the next state
        pred_mean, pred_cov = _predict(filtered_mean, filtered_cov, F, b, Q)
...
def lgssm_smoother(
    ...
    # Run the Kalman filter
    filtered_posterior = lgssm_filter(params, emissions, condition, trial_id)
    ...
    def _step(carry, args):
        ...
        smoothed_mean = filtered_mean + G @ (smoothed_mean_next - F @ filtered_mean - b)
